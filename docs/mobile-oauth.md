...
### 2. Configure Cloudflare Access for the mobile login routes

The mobile OAuth flow uses two routes, both of which need to be protected by Cloudflare Access
(just like the admin pages):

- `/api/mobile-login` - a landing page the app opens in the system browser; it immediately
  redirects to `/api/mobile-auth` once Access has authenticated the user
- `/api/mobile-auth` - reads the Access-authenticated user, issues the JWT, and redirects to the
  `photos://auth/callback` deep link

In your Cloudflare Zero Trust dashboard:

1. Go to Access > Applications
2. Find your photos application
3. Ensure `/api/mobile-login` and `/api/mobile-auth` are protected by the same policy as `/admin/*`

Or create a new policy:

- **Application domain**: `photos.yourdomain.com`
- **Path**: `/api/mobile-login` (add a second rule/path for `/api/mobile-auth`)
...
4. Browser should open to: `https://photos.yourdomain.com/api/mobile-login?state=<random>`
5. Cloudflare Access prompts for authentication
6. The page immediately redirects to `/api/mobile-auth?state=<random>`, which shows "Authentication Successful"
...
[API] Request: GET /api/user/profile Full URL: https://photos.yourdomain.com/api/user/profile
...
- Ensure `/api/mobile-login` and `/api/mobile-auth` are both protected by Cloudflare Access
