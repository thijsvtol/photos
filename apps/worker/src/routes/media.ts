...
/**
 * GET /media/:slug/poster/:photoId.jpg
 * Serves the still-image poster (cover frame) for a VIDEO, so gallery/timeline grids can render
 * a fast <img> instead of mounting a <video> at the MP4 just to paint one frame. The poster is
 * generated by the nightly ffmpeg job and/or captured client-side at upload, stored at
 * poster/<slug>/<id>.jpg. 404s when no poster exists yet — the client falls back to the tiny
 * blur_placeholder in that window. Same access rules and copy-resolution as the preview route.
 */
app.get('/media/:slug/poster/:photoId', async (c) => {
  const slug = c.req.param('slug');
  ...
  try {
    ...
    const photo = await c.env.DB
      .prepare('SELECT source_photo_id, source_event_slug FROM photos WHERE id = ? AND event_id = ?')
      .bind(photoId, event.id)
      .first<{ source_photo_id: string | null; source_event_slug: string | null }>();

    if (!photo) {
      return c.json({ error: 'Photo not found' }, 404);
    }

    // Resolve R2 key: copies point to the source event's storage.
    const r2Slug = photo.source_event_slug ?? slug;
    const r2PhotoId = photo.source_photo_id ?? photoId;

    const object = await c.env.PHOTOS_BUCKET.get(`poster/${r2Slug}/${r2PhotoId}.jpg`);
    if (!object) {
      // No poster generated yet — the client falls back to the blur placeholder.
    ...
    const size = (object as R2ObjectBody).size;
    if (typeof size === 'number') headers['Content-Length'] = String(size);

    return new Response(object.body, { headers });
  } catch (error) {
    console.error('[MEDIA] Error serving poster:', error);
    return c.json({ error: 'Failed to serve poster' }, 500);
  }
});
